...
// Helper: yes/no/unknown checkbox
function YesCheckbox({ id, label, value, onChange }) {
  return (
...
// Helper: min number input
function MinInput({ label, unit, value, onChange, placeholder }) {
  return (
    <div>
...
// Helper: min slider
function MinSlider({ label, unit, value, onChange, max, step = 1 }) {
  return (
    <div>
...
export default function ProductFilters({ filters, setFilters, maxBuyPrice = 150000, maxRentPrice = 250, products = [] }) {
  // Section open states - these persist because this component is NOT remounted
  const [openSections, setOpenSections] = useState({});

  const toggleSection = (key) => setOpenSections(prev => ({ ...prev, [key]: !prev[key] }));

  const updateFilter = (key, value) => setFilters(prev => ({ ...prev, [key]: value }));
  const updateAdv = (key, value) => setFilters(prev => ({
    ...prev,
  ...
  const maxFreshWater = Math.max(...products.map(p => p.bathroom?.fresh_water_l || 0), 100);
  ...
  const filterContent = (
    <div className="space-y-6">
      ...
      <Collapsible open={openSections.vehicle} onOpenChange={() => toggleSection('vehicle')}>
        <SectionHeader>Base Vehicle</SectionHeader>
        <CollapsibleContent className="mt-4 space-y-4">
          ...
          <MinInput label="Min. WLTP Range" unit="km" value={adv.min_wltp_range} onChange={(v) => updateAdv('min_wltp_range', v)} placeholder="e.g. 200" />
          <MinInput label="Min. Battery Size" unit="kWh" value={adv.min_battery} onChange={(v) => updateAdv('min_battery', v)} placeholder="e.g. 75" />
          <MinInput label="Min. Engine Power" unit="kW" value={adv.min_kw} onChange={(v) => updateAdv('min_kw', v)} placeholder="e.g. 100" />
          <MinInput label="Max. Consumption" unit="kWh/100km" value={adv.max_consumption} onChange={(v) => updateAdv('max_consumption', v)} placeholder="e.g. 25" />
          <MinInput label="Min. AC Charge Speed" unit="kW" value={adv.min_ac_charge} onChange={(v) => updateAdv('min_ac_charge', v)} />
          <MinInput label="Min. DC Charge Speed" unit="kW" value={adv.min_dc_charge} onChange={(v) => updateAdv('min_dc_charge', v)} />
          <EnumSelect label="Charger Types" value={adv.charger_types} onChange={(v) => updateAdv('charger_types', v)}
            options={[{value:"Type2",label:"Type2"},{value:"CCS",label:"CCS"},{value:"Type2 / CCS",label:"Type2 / CCS"}]} />
          <MinInput label="Max. DC 20-80% Time" unit="min" value={adv.max_dc_20_80} onChange={(v) => updateAdv('max_dc_20_80', v)} />
          <MinInput label="Max. Speed" unit="km/h" value={adv.min_max_speed} onChange={(v) => updateAdv('min_max_speed', v)} />
          <MinInput label="Max. Turning Circle" unit="m" value={adv.max_turning_circle} onChange={(v) => updateAdv('max_turning_circle', v)} />
          <EnumSelect label="Drive Type" value={adv.drive} onChange={(v) => updateAdv('drive', v)}
            options={[{value:"front",label:"Front"},{value:"rear",label:"Rear"},{value:"4x4",label:"4x4"}]} />
          <MinInput label="Max. Empty Weight" unit="kg" value={adv.max_weight_empty} onChange={(v) => updateAdv('max_weight_empty', v)} />
          <MinInput label="Min. Additional Weight" unit="kg" value={adv.min_additional_weight} onChange={(v) => updateAdv('min_additional_weight', v)} />
          <EnumSelect label="B-License (3.5t)" value={adv.b_license} onChange={(v) => updateAdv('b_license', v)}
            options={[{value:"yes",label:"Yes"},{value:"no",label:"No"}]} />
          <YesCheckbox id="trailer_hitch" label="Trailer Hitch" value={adv.trailer_hitch} onChange={(v) => updateAdv('trailer_hitch', v)} />
          <YesCheckbox id="sliding_doors" label="Sliding Doors" value={adv.sliding_doors} onChange={(v) => updateAdv('sliding_doors', v)} />
          <YesCheckbox id="backdoor" label="Backdoor" value={adv.backdoor} onChange={(v) => updateAdv('backdoor', v)} />
        </CollapsibleContent>
      </Collapsible>

      {/* ═══════════════ CAMPER DETAILS ═══════════════ */}
      <Collapsible open={openSections.camper} onOpenChange={() => toggleSection('camper')}>
        <SectionHeader>Camper Details</SectionHeader>
        <CollapsibleContent className="mt-4 space-y-4">
          <MinSlider label="Min. Range (realistic)" unit="km" value={adv.min_range} onChange={(v) => updateAdv('min_range', v)} max={maxRange} step={10} />
          <MinInput label="Max. Length" unit="mm" value={adv.max_length} onChange={(v) => updateAdv('max_length', v)} placeholder="e.g. 6000" />
          <MinInput label="Max. Height" unit="mm" value={adv.max_height} onChange={(v) => updateAdv('max_height', v)} placeholder="e.g. 2000" />
          <MinInput label="Max. Width" unit="mm" value={adv.max_width} onChange={(v) => updateAdv('max_width', v)} placeholder="e.g. 2100" />
          <MinSlider label="Min. Seats" unit="" value={adv.min_seats} onChange={(v) => updateAdv('min_seats', v)} max={8} step={1} />
          <MinSlider label="Min. Total Storage" unit="L" value={adv.min_storage_total} onChange={(v) => updateAdv('min_storage_total', v)} max={maxStorage} step={50} />
          <YesCheckbox id="popup_roof" label="Pop-Up-Roof" value={adv.popup_roof} onChange={(v) => updateAdv('popup_roof', v)} />
        </CollapsibleContent>
      </Collapsible>
      ...
      <Collapsible open={openSections.interior} onOpenChange={() => toggleSection('interior')}>
        ...
        <CollapsibleContent className="mt-4 space-y-5">
          ...
          <div className="space-y-3">
            ...
            <MinSlider label="Min. Sleeps" unit="" value={adv.min_sleeps} onChange={(v) => updateAdv('min_sleeps', v)} max={6} step={1} />
            <EnumSelect label="Ventilation" value={adv.ventilation} onChange={(v) => updateAdv('ventilation', v)}
            ...
            <EnumSelect label="Mosquito Nets" value={adv.mosquito_nets} onChange={(v) => updateAdv('mosquito_nets', v)}
          ...
          <div className="space-y-3">
            ...
            <EnumSelect label="Swivel Front Seats" value={adv.swivel_seats} onChange={(v) => updateAdv('swivel_seats', v)}
              options={[{value:"no",label:"No"},{value:"1",label:"1"},{value:"2",label:"2"},{value:"3",label:"3"}]} />
            <YesCheckbox id="indoor_table" label="Indoor Table" value={adv.indoor_table} onChange={(v) => updateAdv('indoor_table', v)} />
            <YesCheckbox id="outdoor_table" label="Outdoor Table" value={adv.outdoor_table} onChange={(v) => updateAdv('outdoor_table', v)} />
            <EnumSelect label="ISO-Fix" value={adv.iso_fix} onChange={(v) => updateAdv('iso_fix', v)}
              options={[{value:"no",label:"No"},{value:"front",label:"Front"},{value:"rear",label:"Rear"},{value:"front & rear",label:"Front & Rear"}]} />
            <EnumSelect label="Tinted Windows" value={adv.tinted_windows} onChange={(v) => updateAdv('tinted_windows', v)}
              options={[{value:"yes",label:"Yes"},{value:"partially",label:"Partially"},{value:"no",label:"No"}]} />
            <EnumSelect label="Curtains" value={adv.curtains} onChange={(v) => updateAdv('curtains', v)}
              options={[{value:"no",label:"No"},{value:"front cabin",label:"Front Cabin"},{value:"everywhere",label:"Everywhere"}]} />
            <EnumSelect label="Awning" value={adv.awning} onChange={(v) => updateAdv('awning', v)}
              options={[{value:"yes",label:"Yes"},{value:"keder pre-installed",label:"Keder Pre-installed"},{value:"no",label:"No"}]} />
            <EnumSelect label="Indoor Lights" value={adv.indoor_lights} onChange={(v) => updateAdv('indoor_lights', v)}
          ...
          <div className="space-y-3">
            ...
            <EnumSelect label="Fridge Type" value={adv.fridge_type} onChange={(v) => updateAdv('fridge_type', v)}
              options={[{value:"no",label:"No"},{value:"electric",label:"Electric"},{value:"gas",label:"Gas"}]} />
            <MinSlider label="Min. Fridge Size" unit="L" value={adv.min_fridge} onChange={(v) => updateAdv('min_fridge', v)} max={maxFridge} step={5} />
            <EnumSelect label="Stove Type" value={adv.stove_type} onChange={(v) => updateAdv('stove_type', v)}
              options={[{value:"no",label:"No"},{value:"electric",label:"Electric"},{value:"gas",label:"Gas"}]} />
            <YesCheckbox id="indoor_cooking" label="Indoor Cooking" value={adv.indoor_cooking} onChange={(v) => updateAdv('indoor_cooking', v)} />
            <EnumSelect label="Outdoor Cooking" value={adv.outdoor_cooking} onChange={(v) => updateAdv('outdoor_cooking', v)}
          ...
          <div className="space-y-3">
            ...
            <MinInput label="Min. Fresh Water" unit="L" value={adv.min_fresh_water} onChange={(v) => updateAdv('min_fresh_water', v)} />
            <YesCheckbox id="warm_water" label="Warm Water" value={adv.warm_water} onChange={(v) => updateAdv('warm_water', v)} />
            <EnumSelect label="Shower" value={adv.shower} onChange={(v) => updateAdv('shower', v)}
              options={[{value:"indoor",label:"Indoor"},{value:"outdoor",label:"Outdoor"},{value:"no",label:"No"}]} />
            <YesCheckbox id="warm_shower" label="Warm Shower" value={adv.warm_shower} onChange={(v) => updateAdv('warm_shower', v)} />
            <EnumSelect label="Toilet Type" value={adv.toilet_type} onChange={(v) => updateAdv('toilet_type', v)}
      ...
      <Collapsible open={openSections.energy} onOpenChange={() => toggleSection('energy')}>
        ...
        <CollapsibleContent className="mt-4 space-y-4">
          <MinSlider label="Min. Camping Battery" unit="Wh" value={adv.min_camping_battery} onChange={(v) => updateAdv('min_camping_battery', v)} max={maxCampingBattery} step={100} />
          <YesCheckbox id="solar_panel" label="Solar Panel" value={adv.solar_panel_available} onChange={(v) => updateAdv('solar_panel_available', v)} />
          <MinInput label="Min. Solar Panel Power" unit="W" value={adv.min_solar_w} onChange={(v) => updateAdv('min_solar_w', v)} />
          <MinInput label="Min. USB-C Plugs (Cockpit)" value={adv.min_usb_front} onChange={(v) => updateAdv('min_usb_front', v)} />
          <MinInput label="Min. USB-C Plugs (Living)" value={adv.min_usb_living} onChange={(v) => updateAdv('min_usb_living', v)} />
          <div className="flex items-center space-x-2">
      ...
      {/* ═══════════════ COMFORT (Climate + Smart) ═══════════════ */}
      <Collapsible open={openSections.comfort} onOpenChange={() => toggleSection('comfort')}>
        <SectionHeader>Comfort</SectionHeader>
        <CollapsibleContent className="mt-4 space-y-5">
          {/* Climate */}
          <div className="space-y-3">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Climate</p>
            <YesCheckbox id="ac" label="Air Conditioning" value={adv.ac} onChange={(v) => updateAdv('ac', v)} />
            <YesCheckbox id="climate_prog" label="Climate Programming" value={adv.vehicle_climate_programming} onChange={(v) => updateAdv('vehicle_climate_programming', v)} />
            <EnumSelect label="Vehicle Heating" value={adv.vehicle_heating} onChange={(v) => updateAdv('vehicle_heating', v)}
              options={[{value:"no",label:"No"},{value:"electric",label:"Electric"},{value:"gas",label:"Gas"}]} />
            <EnumSelect label="Vehicle Cooling" value={adv.vehicle_cooling} onChange={(v) => updateAdv('vehicle_cooling', v)}
              options={[{value:"no",label:"No"},{value:"electric",label:"Electric"}]} />
            <EnumSelect label="Stand Heating" value={adv.stand_heating} onChange={(v) => updateAdv('stand_heating', v)}
              options={[{value:"no",label:"No"},{value:"electric",label:"Electric"},{value:"gas",label:"Gas"}]} />
            <YesCheckbox id="living_heating" label="Living Room Heating" value={adv.living_room_heating} onChange={(v) => updateAdv('living_room_heating', v)} />
            <YesCheckbox id="insulation" label="Insulation" value={adv.insulation} onChange={(v) => updateAdv('insulation', v)} />
            <YesCheckbox id="seat_heating" label="Seat Heating" value={adv.seat_heating} onChange={(v) => updateAdv('seat_heating', v)} />
            <YesCheckbox id="steering_heating" label="Steering Wheel Heating" value={adv.steering_wheel_heating} onChange={(v) => updateAdv('steering_wheel_heating', v)} />
          </div>
          <Separator />
          {/* Smart & Connected */}
          <div className="space-y-3">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Smart & Connected</p>
            <YesCheckbox id="remote_app" label="Remote App Access" value={adv.remote_app_access} onChange={(v) => updateAdv('remote_app_access', v)} />
            <EnumSelect label="CarPlay / Android Auto" value={adv.carplay} onChange={(v) => updateAdv('carplay', v)}
              options={[{value:"yes",label:"Yes"},{value:"cable",label:"Cable"},{value:"wireless",label:"Wireless"},{value:"no",label:"No"}]} />
            <EnumSelect label="Navigation" value={adv.navigation} onChange={(v) => updateAdv('navigation', v)}
              options={[{value:"yes",label:"Yes"},{value:"no",label:"No"},{value:"optional",label:"Optional"}]} />
            <EnumSelect label="Cruise Control" value={adv.cruise_control} onChange={(v) => updateAdv('cruise_control', v)}
              options={[{value:"yes",label:"Yes"},{value:"ACC",label:"ACC"},{value:"no",label:"No"}]} />
            <YesCheckbox id="lane_assist" label="Lane Assist" value={adv.lane_assist} onChange={(v) => updateAdv('lane_assist', v)} />
            <YesCheckbox id="sign_recognition" label="Sign Recognition" value={adv.sign_recognition} onChange={(v) => updateAdv('sign_recognition', v)} />
            <YesCheckbox id="rear_camera" label="Rear Camera" value={adv.rear_camera} onChange={(v) => updateAdv('rear_camera', v)} />
            <EnumSelect label="Parking Sensors" value={adv.parking_sensors} onChange={(v) => updateAdv('parking_sensors', v)}
              options={[{value:"no",label:"No"},{value:"front",label:"Front"},{value:"rear",label:"Rear"},{value:"front & rear",label:"Front & Rear"}]} />
          </div>
        </CollapsibleContent>
      </Collapsible>
